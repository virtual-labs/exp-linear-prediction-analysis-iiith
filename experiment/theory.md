...
The short-time spectrum consists of a range of frequencies (magnitude and phase components) present in a small segment (10-30 ms) of a signal. The inverse spectrum $A(j\omega)$ is an all-zero model derived from $A(z)$. The LP spectrum is an all-pole model given by:

$$
H_{LP}(j\omega) = \frac{1}{A(j\omega)}
$$
...
$$
E_r = \sum_{n=0}^{N-1} e^2[n], \quad E_s = \sum_{n=0}^{N-1} s^2[n]
$$
